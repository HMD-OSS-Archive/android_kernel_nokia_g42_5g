...
static ssize_t focaltech_info_show(struct device *dev,
                struct device_attribute *attr, char *buf)
{
    return snprintf(buf, PAGE_SIZE, "%s\n", "FT9391SD");
}
static DEVICE_ATTR_RO(focaltech_info);

static struct attribute *focaltech_attributes[] = {
        &dev_attr_focaltech_info.attr,
        NULL,
};

static const struct attribute_group focaltech_attribute_group = {
        .name = "focaltech",
...
ff_log_level_t g_log_level = 0;//__FF_EARLY_LOG_LEVEL;
...
static const char *ff_ctl_get_version(void)
{
    ...
    /* Version info. */
    //version[0] = '\0';
    snprintf(version, sizeof(FF_DRV_VERSION)+1, FF_DRV_VERSION);
    ...
    snprintf(version, FF_DRV_VERSION_LEN, "%s-%s", version, __FF_BUILD_DATE);
#endif
...
static int ff_ctl_free_driver(struct miscdevice *dev)
{
    int err = 0;
    FF_LOGV("'%s' enter.", __func__);
    ...
    sysfs_remove_group(&dev->this_device->kobj, &focaltech_attribute_group);
    FF_LOGV("'%s' leave.", __func__);
    return err;
}

static int ff_ctl_init_driver(struct miscdevice *dev)
{
    int err = 0;
    FF_LOGV("'%s' enter.", __func__);
    ...
    err = sysfs_create_group(&dev->this_device->kobj, &focaltech_attribute_group);
    printk("Add fingerprint info err=%d", err);

    if (err) {
        ff_ctl_free_driver(dev);
        return err;
    }
...
static long ff_ctl_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
    ...
    if (g_log_level <= FF_LOG_LEVEL_DBG) {
        static const char *cmd_names[] = {
                ...
                "FF_IOC_GET_VERSION", "FF_IOC_GET_IC_INFORMATION",
                "unknown",
        };
    ...
    case FF_IOC_INIT_DRIVER: {
        if (g_context->b_driver_inited) {
            err = ff_ctl_free_driver(dev);
        }
        if (!err) {
            err = ff_ctl_init_driver(dev);
            FF_LOGE("start to init driver");
...
static int __init ff_ctl_driver_init(void)
{
    ...
    if (!ff_ctl_context.wake_lock) {
        FF_LOGE("wakeup_source_register failed.");
        return -ENOMEM;
    }
#else
...
static void __exit ff_ctl_driver_exit(void)
{
    ...
    if (g_context->b_driver_inited) {
        err = ff_ctl_free_driver(&g_context->miscdev);
        g_context->b_driver_inited = false;
    }
