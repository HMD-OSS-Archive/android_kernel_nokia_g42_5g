...
#define FPSENSOR_USE_QCOM_POWER_REG      1
...
/* fingerprint info*/
static ssize_t chipone_info_show(struct device *pdev,
                struct device_attribute *attr, char *buf)
{
    return snprintf(buf, PAGE_SIZE, "%s\n", "ICNF7318CL");
}
static DEVICE_ATTR_RO(chipone_info);

static struct attribute *chipone_attributes[] = {
        &dev_attr_chipone_info.attr,
        NULL,
};

static const struct attribute_group chipone_attribute_group = {
        .name = "chipone",
        .attrs = chipone_attributes,
};
...
static void fpsensor_gpio_free(fpsensor_data_t *fpsensor)
{
    ...
    fpsensor_debug(ERR_LOG, "fpsensor_gpio_free enter\n");
   ...
   fpsensor_debug(ERR_LOG, "fpsensor_gpio_free leave\n");
}
...
#if FPSENSOR_USE_QCOM_POWER_REG
    ret = regulator_enable(g_fpsensor->vdd_reg);
    if (ret){
        fpsensor_debug(ERR_LOG, "failed to enable relulator,value of status is %d", ret);
        regulator_put(g_fpsensor->vdd_reg);
        g_fpsensor->vdd_reg = NULL;
        return -1;
    }else{
        fpsensor_debug(ERR_LOG, "success to enable relulator,value of status is %d", ret);
    }
#endif
...
static int fpsensor_release(struct inode *inode, struct file *filp)
{
    ...
    if (fpsensor_dev->users <= 0) {
        fpsensor_debug(ERR_LOG, "%s, disble_irq. irq = %d\n", __func__, fpsensor_dev->irq);
        fpsensor_disable_irq(fpsensor_dev);
    }
#if FPSENSOR_USE_QCOM_POWER_REG
    regulator_disable(g_fpsensor->vdd_reg);
    //regulator_put(g_fpsensor->vdd_reg);
#endif
    fpsensor_dev->device_available = 0;
    FUNC_EXIT();
...
#endif
    g_fpsensor->vdd_reg = regulator_get(&g_fpsensor->spi->dev,"finger_vio28");
    //regulator_set_voltage(g_fpsensor->vdd_reg, 2800000, 2900000);
    if(IS_ERR(g_fpsensor->vdd_reg)){
        status = PTR_ERR(g_fpsensor->vdd_reg);
	g_fpsensor->vdd_reg = NULL;
	fpsensor_debug(ERR_LOG, "failed to get g_fpsensor->vdd_reg,value of status is %d", status);
	goto release_drv_data;
    }else{
        fpsensor_debug(ERR_LOG, "success to get g_fpsensor->vdd_reg");
    }
    status = regulator_set_voltage(g_fpsensor->vdd_reg, 2800000, 2900000);
    if (status){
        regulator_put(g_fpsensor->vdd_reg);
	g_fpsensor->vdd_reg = NULL;
	fpsensor_debug(ERR_LOG, "failed to set voltage for finger_vio28,of status is %d", status);
        goto release_drv_data;
    }else{
        fpsensor_debug(ERR_LOG, "success to set voltage for finger_vio28,of status is %d", status);
    }
    status = regulator_enable(g_fpsensor->vdd_reg);
    if (status){
        fpsensor_debug(ERR_LOG, "failed to enable relulator,value of status is %d", status);
        regulator_put(g_fpsensor->vdd_reg);
        g_fpsensor->vdd_reg = NULL;
	goto release_drv_data;
    }else{
        fpsensor_debug(ERR_LOG, "success to enable relulator,value of status is %d", status);
    }
    status = regulator_disable(g_fpsensor->vdd_reg);
    if (status){
        fpsensor_debug(ERR_LOG, "failed to disable relulator,value of status is %d", status);
        regulator_put(g_fpsensor->vdd_reg);
        g_fpsensor->vdd_reg = NULL;
        goto release_drv_data;
    }else{
        fpsensor_debug(ERR_LOG, "success to disable relulator,value of status is %d", status);
    }

    sysfs_create_group(&pdev->dev.kobj, &chipone_attribute_group);
...
static int fpsensor_remove(struct platform_device *pdev)
{
    ...
    kfree(fpsensor_dev);
    g_fpsensor = NULL;
	sysfs_remove_group(&pdev->dev.kobj, &chipone_attribute_group);
    FUNC_EXIT();
    return 0;
}
...
static struct of_device_id fpsensor_of_match[] = {
    { .compatible = "qcom,fingerprint-fpsensor" },
    {}
};
