...
//extern const char *lcm_info;
const char *lcm_info = NULL;
	...
	if (tcm_hcd->report.id == REPORT_TOUCH) {
		...
		if ((tcm_hcd->report.id == REPORT_IDENTIFY) &&
			...
			if (atomic_read(&tcm_hcd->helper.task) == HELP_NONE) {
				/*LOGN(tcm_hcd->pdev->dev.parent,"isr queue hdl work HELP_TOUCH_REINIT\n");*/
...
/*		LOGN(tcm_hcd->pdev->dev.parent,
		...
		if ((tcm_hcd->id_info.mode == MODE_ROMBOOTLOADER) &&
			...
			if (atomic_read(&tcm_hcd->helper.task) ==
					HELP_NONE) {
				/*LOGN(tcm_hcd->pdev->dev.parent,"isr queue hdl work HELP_SEND_ROMBOOT_HDL\n");*/
				atomic_set(&tcm_hcd->helper.task,
...
static int ovt_tcm_enable_irq(struct ovt_tcm_hcd *tcm_hcd, bool en, bool ns)
{
	...
	LOGN(tcm_hcd->pdev->dev.parent,"ovt_tcm_enable_irq  en:%s\n", en ? "true" : "false");
...
static int ovt_tcm_resume(struct device *dev)
{
	...
	LOGN(tcm_hcd->pdev->dev.parent,"ovt_tcm_resume, to queue speed up work\n");
...
static int ovt_tcm_suspend(struct device *dev)
{
	...
	LOGN(tcm_hcd->pdev->dev.parent,"ovt_tcm_suspend enter\n");
	if (tcm_hcd->in_suspend || tcm_hcd->ovt_tcm_driver_removing)
		return 0;
	...
	mutex_unlock(&tcm_hcd->suspend_resume_mutex);
	LOGN(tcm_hcd->pdev->dev.parent,"ovt_tcm_suspend exit\n");
    return 0;
}
#endif
#ifdef CONFIG_DRM
static int ovt_tcm_early_suspend(struct device *dev)
{
	...
	struct ovt_tcm_hcd *tcm_hcd = dev_get_drvdata(dev);
	int retval;
	LOGN(tcm_hcd->pdev->dev.parent,"ovt_tcm_early_suspend enter\n");
	if (tcm_hcd->in_suspend  || tcm_hcd->ovt_tcm_driver_removing)
		return 0;
...
static int ovt_tcm_early_suspend(struct device *dev)
{
	...
	LOGN(tcm_hcd->pdev->dev.parent,"ovt_tcm_early_suspend (fb) enter\n");
	if (tcm_hcd->in_suspend  || tcm_hcd->ovt_tcm_driver_removing)
		return 0;
...
#endif
	if (lcm_info == NULL) {
		printk("omnivision lcm info is null");
		return -ENODEV;
	}
	if (strstr(lcm_info, "qcom,mdss_dsi_td4160_90hz_hd_hx_video") == NULL)
		return -ENODEV;
