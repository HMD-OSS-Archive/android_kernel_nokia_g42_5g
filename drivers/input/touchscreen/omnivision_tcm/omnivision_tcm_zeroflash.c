...
static void ontim_dev_get_omnvi_info(int);
static char version[50]="unknown";
static char vendor_name[20]="unknown";
static char lcdname[20]="unknown";
...
#endif
/* added by zhangshaohu for shadow-132 (20221228) end */

#define ENABLE_SYS_ZEROFLASH true

#define FW_IMAGE_NAME "hx_td4160.img"
...
#if CONFIG_ONTIM_HWINFO_SUPPORT
static void ontim_dev_get_omnvi_info(int tpfw_version)
{
	...
	REGISTER_AND_INIT_ONTIM_DEBUG_FOR_THIS_DEV();
		...
		snprintf(lcdname, sizeof(lcdname), "hx-td4160");
		snprintf(vendor_name, sizeof(vendor_name), "hx-td4160");
		snprintf(version, sizeof(version),"hx-td4160-version#fw:%d_VID:0x95", tpfw_version);
		//snprintf(version, sizeof(version),"hx-td4160-version#fw:35150_VID:0x95");
}
#endif
...
static int zeroflash_parse_fw_image(void)
{
	...
	for (idx = 0; idx < num_of_areas; idx++) {
		...
		if (0 == strncmp((char *)descriptor->id_string,
			...
			#if CONFIG_ONTIM_HWINFO_SUPPORT
				ontim_dev_get_omnvi_info(image_info->packrat_number);
			#endif
			LOGD(tcm_hcd->pdev->dev.parent,
