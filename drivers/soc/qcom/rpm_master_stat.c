...
struct msm_rpm_master_stats_platform_data {
	...
	// ning.wei@hmd++
	#ifdef CONFIG_HMD_PM_DEBUG
	struct kobj_attribute hmd_ka;
	struct kobject *kobj;
	struct dentry *dent;
	#endif
};
...
size_t hmd_get_rpm_master_stats(char *buf, size_t size)
{
	...
	for (i = 0; i < prvdata->num_masters; i++) {
		...
		if (buf) {
			SNPRINTF(buf, size, "%s:0x%x:0x%llx\n",
...
EXPORT_SYMBOL(hmd_get_rpm_master_stats);

static ssize_t hmd_msm_rpm_master_stats_show(struct kobject *kobj,
				struct kobj_attribute *attr, char *buf)
{
	return hmd_get_rpm_master_stats(buf, RPM_MASTERS_BUF_LEN);
}
...
static  int msm_rpm_master_stats_probe(struct platform_device *pdev)
{
	...
	sysfs_attr_init(&pdata->hmd_ka.attr);
	pdata->hmd_ka.attr.mode = 0444;
	pdata->hmd_ka.attr.name = "hmd_rpm_master_stats";
	pdata->hmd_ka.show = hmd_msm_rpm_master_stats_show;
	pdata->hmd_ka.store = NULL;

	ret = sysfs_create_file(pdata->kobj, &pdata->hmd_ka.attr);
	if (ret) {
...
static int msm_rpm_master_stats_remove(struct platform_device *pdev)
{
	...
	if( g_pdata != NULL){
		sysfs_remove_file(pdata->kobj, &pdata->hmd_ka.attr);
