...
#include "ispreg.h"
#include "ispcsi2.h"
//guozy add cam_err node for debug 20220831 start
#ifdef CAM_ERR_CODE
#include <camera_err_code.h>
#endif
//guozy add cam_err node for debug 20220831 end
/*
...
//guozy add cam_err node for debug 20220831 start
#ifdef CAM_ERR_CODE
		cam_err_code |= (CAM_QCOM_PLATFORM | ISP_CSI2_COMPLEXIO1_ERR_IRQ);
		dev_dbg(isp->dev, "cam_err_code:0x%x\n", cam_err_code);
#endif
//guozy add cam_err node for debug 20220831 end
		pipe->error = true;
	}
...
//guozy add cam_err node for debug 20220831 start
#ifdef CAM_ERR_CODE
		cam_err_code |= CAM_QCOM_PLATFORM;
		if (csi2_irqstatus & ISPCSI2_IRQSTATUS_OCP_ERR_IRQ)
			cam_err_code |= ISP_CSI2_OCP_ERR_IRQ;
		if (csi2_irqstatus & ISPCSI2_IRQSTATUS_SHORT_PACKET_IRQ)
			cam_err_code |= ISP_CSI2_SHORT_PACKET_IRQ;
		if (csi2_irqstatus & ISPCSI2_IRQSTATUS_ECC_NO_CORRECTION_IRQ)
			cam_err_code |= ISP_CSI2_ECC_NO_CORRECTION_IRQ;
		if (csi2_irqstatus & ISPCSI2_IRQSTATUS_COMPLEXIO2_ERR_IRQ)
			cam_err_code |= ISP_CSI2_COMPLEXIO2_ERR_IRQ;
		if (csi2_irqstatus & ISPCSI2_IRQSTATUS_FIFO_OVF_IRQ)
			cam_err_code |= ISP_CSI2_FIFO_OVF_IRQ);
		dev_dbg(isp->dev, "cam_err_code:0x%x\n", cam_err_code);
#endif
//guozy add cam_err node for debug 20220831 end
		pipe->error = true;
