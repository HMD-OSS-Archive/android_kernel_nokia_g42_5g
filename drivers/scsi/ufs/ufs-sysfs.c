...
static ssize_t ufs_capacity_show(struct device *dev,
                struct device_attribute *attr, char *buf)
{
	int ret;
	int i;
	int capacity[] = { 0, 4, 8, 16, 32, 64, 128, 256, 512 };
	unsigned long long size = 0;
	char str[24] = { 0 };
	ret = raw_device_capacity_show(dev, NULL, str);
	if (ret > 0)
		kstrtoull(str, 0, &size);
	// convert from sectors to G
	size = CEIL(size, 1024 * 1024 * 2);
	for (i = 0; i < ARRAY_SIZE(capacity); i++)
		if (size <= capacity[i])
			break;
	ret = sprintf(buf, "%dG\n", (i < ARRAY_SIZE(capacity)) ? capacity[i] : (int)size);
	pr_info("%s: raw=%s sectors (%dG)  ufs_capacity=%s", __func__, str, (int)size, buf);
	return ret;
}
static DEVICE_ATTR_RO(ufs_capacity);
...
{									\
	...
	ret = ufshcd_read_string_desc(hba, index, &desc_buf,		\
				      !isSN);			\
	dev_info(dev, "param=DEVICE_DESC_PARAM" #_pname "(0x%x) index=%d ret=%d\n",\
		DEVICE_DESC_PARAM##_pname, index, ret); \
	if (ret < 0)							\
		goto out;						\
	if (isSN) {							\
		int i = 0;						\
		int n = 0;						\
		int delta = 1;						\
		u8 *p = desc_buf + QUERY_DESC_HDR_SIZE;			\
		ret = (desc_buf[0] - QUERY_DESC_HDR_SIZE);		\
		if (!p[0] && !p[2] && !p[4] && !p[6])			\
			delta = 2;					\
		if (ret > MAX_SERIAL_NUMBER_LEN / 2 * delta)		\
			ret = MAX_SERIAL_NUMBER_LEN / 2 * delta;	\
		for (i = 0; i < ret; i += delta)			\
			n += sprintf(buf + n, "%02x", p[i + delta - 1]);\
		buf[n++] = '\n';					\
		buf[n] = 0;						\
		ret = n;						\
	} else {							\
...
typedef ssize_t (* func_show_t)(struct device *dev, struct device_attribute *attr, char *buf);
static ssize_t ufs_info_show(struct device *dev,
                struct device_attribute *attr, char *buf)
{
	int i;
	int ret;
	...
	func_show_t func[] = { manufacturer_name_show, product_name_show, product_revision_show, manufacturing_date_show };
	for (i = 0; i < ARRAY_SIZE(func); i++, n++) {
		ret = func[i](dev, NULL, buf + n);
		if (ret <= 0) {
			strcpy(buf + n, UNKNOWN);
			ret = sizeof(UNKNOWN) - 1;
		}
		n += ret;
		for (n--; n >= 0; n--) {
			char ch = buf[n];
...
static DEVICE_ATTR_RO(ufs_info);

#define MEMORY_CID_LEN 32
static ssize_t ufs_cid_show(struct device *dev,
                struct device_attribute *attr, char *buf)
{
	int len;
	len = serial_number_show(dev, NULL, buf);
	if (len < 0)
		len = 0;
	if (len < MEMORY_CID_LEN)
		memset(buf + len, 'f', MEMORY_CID_LEN - len);
	buf[MEMORY_CID_LEN] = '\n';
	buf[MEMORY_CID_LEN + 1] = 0;
	pr_info("%s: %s\n", __func__, buf);
	return MEMORY_CID_LEN + 1;
}
static DEVICE_ATTR_RO(ufs_cid);
...
static struct attribute *ufs_sysfs_string_descriptors[] = {
	...
	&dev_attr_ufs_info.attr,
	&dev_attr_ufs_cid.attr,
	NULL,
};
