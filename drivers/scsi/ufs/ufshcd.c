...
static char powp_type[64] = {0};
static int __init get_powp_type(char *line)
{
		
		
		strlcpy(powp_type, line, sizeof(powp_type));
			if (!strcmp(powp_type, "Enable")) {
						powp_flag = 1;
							}
				return 1;
}

__setup("androidboot.powpstate=", get_powp_type);
static int ufshcd_device_params_init(struct ufs_hba *hba)
{
	...
	if(powp_flag) {
		if(!ufshcd_query_flag_retry(hba, UPIU_QUERY_OPCODE_SET_FLAG,QUERY_FLAG_IDN_PWR_ON_WPE, 0, NULL)) {
			pr_err("The kernel enables partition write protection successfully");
		}
	}
