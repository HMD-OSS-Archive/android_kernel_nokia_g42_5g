...
#endif
#ifdef CONFIG_HYPERHOLD_CORE
static unsigned long hyperhold_init_val = 0;
#endif
...
#ifdef CONFIG_HYPERHOLD_CORE
static ssize_t hyperhold_init_show(struct device *dev,
struct device_attribute *attr, char *buf)
{
...
#if 0
#ifdef CONFIG_HYPERHOLD_CORE
    pr_info("huanghao hyperhold_init!!!\n");
	hyperhold_init(zram);
#endif
#endif
	return len;
...
#ifdef CONFIG_HYPERHOLD_CORE
static ssize_t hyperhold_init_store(struct device *dev,
struct device_attribute *attr, const char *buf, size_t len)
{
	int ret;
	struct zram *zram = dev_to_zram(dev);
	ret = kstrtoul(buf, 0, &hyperhold_init_val);
	if (unlikely(ret)) {
		pr_err("val is error!\n");
		return -EINVAL;
	}
	if((int)hyperhold_init_val == 1)
	{
		if (zram->disksize == 0) {
			pr_err("zram disksize is 0\n");
			return -EINVAL;
		}
		else
		{
...
#endif
#ifdef CONFIG_HYPERHOLD_CORE
static DEVICE_ATTR_RW(hyperhold_init);
