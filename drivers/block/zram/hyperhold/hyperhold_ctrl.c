...
static int hyperhold_bind(struct zram *zram, const char *file_name)
{
	...
	eswap_size = (unsigned long long)i_size_read(inode);
	atomic64_set(&stat->eswap_size, eswap_size);
	...
	struct device_attribute *attr, char *buf)
{
	...
	if (stat)
	{
		...
		size += scnprintf(buf + size, PAGE_SIZE - size,
				"Hyperhold zram used: %ld\n",
				atomic64_read(&stat->zram_stored_size) >> MBYTE_SHIFT);
		size += scnprintf(buf + size, PAGE_SIZE - size,
				"Hyperhold zram pagesize: %ld\n",
				atomic64_read(&stat->zram_stored_pages) >> (MBYTE_SHIFT - PAGE_SHIFT));
		size += scnprintf(buf + size, PAGE_SIZE - size,
				"Hyperhold memory page size: %ld\n",
				atomic64_read(&stat->stored_pages) >> (MBYTE_SHIFT - PAGE_SHIFT));
		size += scnprintf(buf + size, PAGE_SIZE - size,
				"Hyperhold eswap used: %ld\n",
				atomic64_read(&stat->stored_size) >> MBYTE_SHIFT);
		size += scnprintf(buf + size, PAGE_SIZE - size,
				"Hyperhold eswap size: %ld\n",
				atomic64_read(&stat->eswap_size) >> MBYTE_SHIFT);
	}
