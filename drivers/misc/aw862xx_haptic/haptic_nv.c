...
#define HAPTIC_NV_DRIVER_VERSION	"v1.10.0"
...
static unsigned int hapticf0 = 0x00;
...
static void ram_load(const struct firmware *cont, void *context)
{
	...
	struct aw_haptic_container *aw_fw;
#if !defined(BUILD_USER_TYPE_VERSION)
...
static void haptic_audio_work_routine(struct work_struct *work)
{
	...
	if (aw_haptic->haptic_audio.ctr.cmd & AW_CMD_R_EN) {
		...
		right->func->set_wav_seq(right, 0x00,
			aw_haptic->haptic_audio.ctr.wavseq);
		right->func->set_wav_seq(right, 0x01, 0x00);
		right->func->set_wav_loop(right, 0x00,
			aw_haptic->haptic_audio.ctr.loop);
		right->func->set_gain(right,
			aw_haptic->haptic_audio.ctr.gain);
	}
	/* play go */
	if (aw_haptic->haptic_audio.ctr.cmd & AW_CMD_L_EN)
			       ...
			       struct device_attribute *attr, char *buf)
{
	...
	while (i < aw_haptic->ram.len) {
		...
		for (j = 0; j < size; j++) {
			...
			//strcat(buffer, unit);
			strncpy(buffer + (6 * (j % AW_RAMDATA_SHOW_COLUMN)), unit, 6);
...
static DEVICE_ATTR(vbat_monitor, S_IWUSR | S_IRUGO, vbat_monitor_show, vbat_monitor_store);
static DEVICE_ATTR(lra_resistance, S_IWUSR | S_IRUGO, lra_resistance_show,
		   lra_resistance_store);
static DEVICE_ATTR(prctmode, S_IWUSR | S_IRUGO, prctmode_show, prctmode_store);
static DEVICE_ATTR(ram_vbat_comp, S_IWUSR | S_IRUGO, ram_vbat_comp_show,
		   ram_vbat_comp_store);
static DEVICE_ATTR(ram_num, S_IRUGO, ram_num_show, NULL);
...
static DEVICE_ATTR(haptic_audio, S_IWUSR | S_IRUGO, haptic_audio_show, haptic_audio_store);
#ifdef AW_DOUBLE
static DEVICE_ATTR(dual_cont, S_IWUSR, NULL, dual_cont_store);
			...
			const struct i2c_device_id *id)
{
	...
	if (gpio_is_valid(aw_haptic->reset_gpio)) {
		ret = gpio_request_one(aw_haptic->reset_gpio,
	...
	if (gpio_is_valid(aw_haptic->irq_gpio)) {
		ret = gpio_request_one(aw_haptic->irq_gpio,
	...
	aw_haptic->f0_cali_data = hapticf0;
...
#if 0
static int __maybe_unused get_hapticf0(char *str)
{
  	get_option(&str, &hapticf0);
  	return 1;
}

__setup("hapticf0=", get_hapticf0);
#endif
...
module_param(hapticf0, uint, 0644);
MODULE_PARM_DESC(hapticf0, "hapticf0 is haptic cali f0 in uefi");
