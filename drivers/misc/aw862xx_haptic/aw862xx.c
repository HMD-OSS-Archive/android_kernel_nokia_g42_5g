...
static void aw862xx_parse_dts(struct aw_haptic *aw_haptic,
			     struct device_node *np)
{
	uint32_t duration_time[3];
	uint32_t trig_config_temp[21];
	uint32_t sine_array_temp[4];
	uint32_t val = 0;

	val = of_property_read_u32(np, "aw862xx_gain_bypass",
				  &aw_haptic->info.gain_bypass);
	if (val != 0)
		aw_info("aw862xx_gain_bypass not found");
	val = of_property_read_u32(np, "aw862xx_vib_lk_f0_cali",
				  &aw_haptic->info.lk_f0_cali);
	if (val != 0)
		aw_info("aw862xx_vib_lk_f0_cali not found");
	val = of_property_read_u32(np, "aw862xx_vib_mode",
				  &aw_haptic->info.mode);
	if (val != 0)
		aw_info("aw862xx_vib_mode not found");
	val = of_property_read_u32(np, "aw862xx_vib_f0_pre",
	...
	val = of_property_read_u32(np, "aw862xx_vib_cont_drv1_lvl",
				  &aw_haptic->info.cont_drv1_lvl);
	if (val != 0)
		aw_info("vib_cont_drv1_lvl not found");
	val = of_property_read_u32(np, "aw862xx_vib_lra_vrms",
				   &aw_haptic->info.lra_vrms);
	if (val != 0)
		aw_info("vib_cont_lra_vrms not found");
	val = of_property_read_u32(np, "aw862xx_vib_cont_brk_time",
				  &aw_haptic->info.cont_brk_time);
	if (val != 0)
		aw_info("vib_cont_brk_time not found");
	val = of_property_read_u32(np, "aw862xx_vib_cont_tset",
				  &aw_haptic->info.cont_tset);
	if (val != 0)
		aw_info("vib_cont_tset not found");
	val = of_property_read_u32(np, "aw862xx_vib_cont_bemf_set",
				  &aw_haptic->info.cont_bemf_set);
	if (val != 0)
		aw_info("vib_cont_bemf_set not found");
	val = of_property_read_u32(np, "aw862xx_vib_cont_wait_num",
				  &aw_haptic->info.cont_wait_num);
	if (val != 0)
		aw_info("vib_cont_wait_num not found");
	val = of_property_read_u32(np, "aw862xx_vib_cont_brk_gain",
				  &aw_haptic->info.cont_brk_gain);
	if (val != 0)
	...
	val = of_property_read_u32_array(np, "aw862xx_vib_sine_array",
				  sine_array_temp, ARRAY_SIZE(sine_array_temp));
	if (val != 0)
		aw_info("aw862xx_vib_sine_array not found");
	else
		memcpy(aw_haptic->info.sine_array, sine_array_temp,
		       sizeof(sine_array_temp));
	val = of_property_read_u32(np, "aw862xx_vib_d2s_gain",
				  &aw_haptic->info.d2s_gain);
	if (val != 0)
		aw_info("vib_d2s_gain not found");
	val = of_property_read_u32_array(np, "aw862xx_vib_trig_config",
					trig_config_temp,
	...
	val = of_property_read_u32_array(np, "aw862xx_vib_duration_time",
					duration_time,
