...
static void msm_gpio_sleep_dbg_show_one(
				  struct gpio_chip *chip,
				  unsigned offset,
				  unsigned gpio)
{
	...
	pr_info("%-5d | %-6d | %-8s | %-6d | %-12s | %-11d | %-12s\n",
				offset, func, ( is_out ? "out" : "in"),
				val,(pctrl->soc->pull_no_keeper ? pulls_no_keeper[pull] : pulls_keeper[pull]),
				msm_regval_to_drive(drive),
				(desc && desc->label ? desc->label : "none"));

}


void msm_gpio_sleep_dbg_show()
{
	...
	pr_info(" ======================= Dump GPIO status begin  =======================\n");
	pr_info("%-5s | %-6s | %-8s | %-6s | %-12s | %-11s | %-12s\n", "id", "func", "direct", "value", "pull", "drv_strenth", "requested by");
	pr_info("------|--------|----------|--------|--------------|-------------|--------------\n");
	for (i = 0; i < chip->ngpio; i++, gpio++)
		msm_gpio_sleep_dbg_show_one(chip, i, gpio);

	pr_info(" ======================= Dump GPIO status end    =======================\n");
}
EXPORT_SYMBOL(msm_gpio_sleep_dbg_show);
#endif
