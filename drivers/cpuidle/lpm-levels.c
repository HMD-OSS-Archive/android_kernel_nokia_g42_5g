...
static bool hmd_lpm_sleep_debug = false;
module_param_named(hmd_lpm_sleep_debug, hmd_lpm_sleep_debug, bool, 0664);
extern void msm_gpio_sleep_dbg_show(void);
extern size_t hmd_get_rpm_master_stats(char *buf, size_t size);
#endif
		...
		bool from_idle, int predicted)
{
	...
	if (level->notify_rpm) {
		...
		if (!from_idle) {
			// ning.wei@hmd++
			#ifdef CONFIG_HMD_PM_DEBUG
			if (hmd_lpm_sleep_debug) {
				msm_gpio_sleep_dbg_show();
				clock_debug_print_enabled();
				regulator_debug_print_enabled();
				pr_info("============ print rpm stats suspend enter =========\n");
				hmd_get_rpm_master_stats(NULL, 0);
				pr_info("===============================================\n");
			}
			#else
			clock_debug_print_enabled();
...
static void lpm_suspend_wake(void)
{
	...
	#ifdef CONFIG_HMD_PM_DEBUG
	if (hmd_lpm_sleep_debug) {
		pr_info("============ print rpm stats suspend exit =========\n");
		hmd_get_rpm_master_stats(NULL, 0);
		pr_info("=============================================\n");
	}
	#endif
