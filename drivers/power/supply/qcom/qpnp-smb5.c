...
static ssize_t chargers_control_show(struct device *dev, struct device_attribute
				*attr, char *buf)
{
	struct smb5 *chip = dev_get_drvdata(dev);
	struct smb_charger *chg = &chip->chg;
...
static ssize_t chargers_control_store(struct device *dev, struct device_attribute
				 *attr, const char *buf, size_t count)
{
	int val;
	struct smb5 *chip = dev_get_drvdata(dev);
	...
	if (!chg->sgm_psy)
		chg->sgm_psy = power_supply_get_by_name("sgm4151x-charger");

	if (!chg->sgm_psy)
		return -EINVAL;

	//enable pm7250b charging
	if(val == 1){
		pval.intval = 0;
		power_supply_set_property(chg->batt_psy, POWER_SUPPLY_PROP_REVERSE_CHG_MODE, &pval);
	}
	//enable sgm charging
	else if(val == 2){
		pval.intval = 500000;
		power_supply_set_property(chg->sgm_psy, POWER_SUPPLY_PROP_CONSTANT_CHARGE_CURRENT, &pval);
		pval.intval = 500000;
		power_supply_set_property(chg->sgm_psy, POWER_SUPPLY_PROP_INPUT_CURRENT_LIMIT, &pval);
		pval.intval = 1;
		power_supply_set_property(chg->sgm_psy, POWER_SUPPLY_PROP_STATUS, &pval);
	}
	//disable pm7250b charging
	else if(val == 3){
		pval.intval = 1;
		power_supply_set_property(chg->batt_psy, POWER_SUPPLY_PROP_REVERSE_CHG_MODE, &pval);
	}
	//disable sgm charging
	else if(val == 4){
		pval.intval = 0;
		power_supply_set_property(chg->sgm_psy, POWER_SUPPLY_PROP_STATUS, &pval);
	}
...
static DEVICE_ATTR_RW(chargers_control);
...
static ssize_t typec_cc_orientation_show(struct device *dev, struct device_attribute
				    *attr, char *buf)
{
	struct smb5 *chip = dev_get_drvdata(dev);
	struct smb_charger *chg = &chip->chg;
...
static struct attribute *smb5_attrs[] = {
	...
	&dev_attr_chargers_control.attr,
...
static enum power_supply_property smb5_usb_props[] = {
	...
	POWER_SUPPLY_PROP_POWER_AVG, // add for turn on/off otg
};
...
static enum power_supply_property smb5_batt_props[] = {
	...
	POWER_SUPPLY_PROP_AUTHENTIC, // add for CIT charger_ic_mfr
	POWER_SUPPLY_PROP_CALIBRATE, // add for CIT typec_mfr
	POWER_SUPPLY_PROP_REVERSE_CHG_MODE, // add for charging suspend
};
		...
		union power_supply_propval *pval)
{
	...
	case POWER_SUPPLY_PROP_CALIBRATE:
		pval->intval = POWER_SUPPLY_TECHNOLOGY_UNKNOWN;
		break;
	...
	case POWER_SUPPLY_PROP_REVERSE_CHG_MODE:
		rc = smblib_get_prop_charging_suspend(chg, pval);
		break;
		...
		const union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_INPUT_CURRENT_LIMIT:
		...
		break;
	case POWER_SUPPLY_PROP_REVERSE_CHG_MODE:
		rc = smblib_set_prop_charging_suspend(chg, val);
		break;
	case POWER_SUPPLY_PROP_CHARGE_CONTROL_LIMIT:
		rc = smblib_set_prop_system_temp_level(chg, val);
		//pr_info("ontim: system_temp_level = %d\n", val->intval);
		break;
	case POWER_SUPPLY_PROP_CAPACITY:
...
static int smb5_init_hw(struct smb5 *chip)
{
	...
	pr_info("ontim: reg 0x154a = 0x%02x\n", val);
	//add end
...
static int smb5_determine_initial_status(struct smb5 *chip)
{
	...
	msleep(200); //The startup speed is fast, so the USB insertion is not detected
