...
#ifndef SMT_VERSION
#define ONLINE_CUTOFF_UV 3200000
static bool is_usb_port_available(struct qpnp_qg *chip)
{
	if (!chip->usb_port_psy)
		chip->usb_port_psy =
			power_supply_get_by_name("pc_port");

	return !!chip->usb_port_psy;
}

static int get_charger_online(struct qpnp_qg *chip){
	union power_supply_propval pval = {0, };
	int usb_online, usb_port_online = 0;

	power_supply_get_property(chip->usb_psy, POWER_SUPPLY_PROP_ONLINE, &pval);
	usb_online = pval.intval;
	qg_dbg(chip, QG_DEBUG_SOC, "ontim: usb_online = %d;\n", usb_online);

	if(is_usb_port_available(chip)) {
		power_supply_get_property(chip->usb_port_psy, POWER_SUPPLY_PROP_ONLINE, &pval);
		usb_port_online = pval.intval;
		qg_dbg(chip, QG_DEBUG_SOC, "ontim: usb_port_online = %d;\n", usb_port_online);
	}

	if(usb_port_online || usb_online)
		return 1;
...
#else
		if(POWER_SUPPLY_STATUS_CHARGING == get_charger_online(chip)) {
			if (!rc && (vbat_uv >= (ONLINE_CUTOFF_UV + VBAT_LOW_HYST_UV))) {//plug charger
				qg_dbg(chip, QG_DEBUG_SOC, "vbat_uv=%duV holding SOC to 1%\n",
						vbat_uv);
				soc = 1;
			}
