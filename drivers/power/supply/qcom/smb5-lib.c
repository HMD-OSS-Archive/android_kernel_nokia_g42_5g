...
int smblib_vbus_regulator_enable(struct regulator_dev *rdev)
{
	...
	if(chg->otg_en){
		...
		smblib_dbg(chg, PR_OTG, "otg_en have be disabled\n");
...
int smblib_get_prop_charging_suspend(struct smb_charger *chg,
				union power_supply_propval *val)
{
...
int smblib_set_prop_charging_suspend(struct smb_charger *chg,
				const union power_supply_propval *val)
{
	int rc;
	...
	if (rc < 0) {
		smblib_err(chg, "Couldn't vote to %s ichg rc=%d\n",
			(bool)val->intval ? "suspend" : "resume", rc);
		return rc;
	}
				...
				const union power_supply_propval *val)
{
	...
	pr_info("ontim: temp_level = %d; temp_level_current = %d\n",
...
int smblib_dp_dm(struct smb_charger *chg, int val)
{
	...
	case QTI_POWER_SUPPLY_DP_DM_DP_PULSE:
		...
		smblib_dbg(chg, PR_MISC, "DP_DM_DP_PULSE rc=%d cnt=%d\n",
				rc, chg->pulse_cnt);
		break;
	case QTI_POWER_SUPPLY_DP_DM_DM_PULSE:
		...
		if (!rc && chg->pulse_cnt)
			chg->pulse_cnt--;
		smblib_dbg(chg, PR_MISC, "DP_DM_DM_PULSE rc=%d cnt=%d\n",
				rc, chg->pulse_cnt);
		break;
	...
	if(target_icl_ua > 1500000){
		...
		smblib_dbg(chg, PR_MISC, "ICL DOWN ICL=%d reduction=%d\n",
				    ...
				    int val)
{
	...
	if (chg->pd_active) {
		...
		pr_info("pd_current_max = %d\n", val);
		rc = vote(chg->usb_icl_votable, PD_VOTER, true, val);
		if (rc < 0){
			smblib_err(chg, "Couldn't set PD ICL rc = %d\n", rc);
			return rc;
		}
...
irqreturn_t smb5_usbin_ov_irq_handler(int irq, void *data)
{
	...
	/* Workaround for non-QC2.0-compliant chargers follows */
	if (apsd->val == QTI_POWER_SUPPLY_TYPE_USB_HVDCP) {
		pr_info("Workaround for non-QC2.0-compliant chargers\n");
		rc = smblib_read(chg, HVDCP_PULSE_COUNT_MAX_REG, &max_pulses);
		if (rc < 0)
			smblib_err(chg,
				"Couldn't read QC2 max pulses rc=%d\n", rc);
