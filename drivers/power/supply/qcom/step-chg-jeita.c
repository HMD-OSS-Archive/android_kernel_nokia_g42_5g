...
static bool is_sgm_available(struct step_chg_info *chip)
{
	if (!chip->sgm_psy)
		chip->sgm_psy = power_supply_get_by_name("sgm4151x-charger");

	if (!chip->sgm_psy)
		chip->sgm_psy = power_supply_get_by_name("eta6963-charger");
...
static int get_step_chg_jeita_setting_from_profile(struct step_chg_info *chip)
{
	...
	if (!rc) {
		...
		pr_err("jeita-fcc-hys: rise_hys=%u, fall_hys=%u\n",
			hysteresis[0], hysteresis[1]);
	}
#ifdef DUAL_85_VERSION
	rc = read_range_data_from_node(profile_node,
			"ontim,jeita-fv-ranges",
	...
	if (of_property_read_bool(profile_node, "qcom,jeita-fcc-scaling")) {
		...
		if (rc < 0)
			pr_err("Read jeita-fcc-scaling-temp-threshold from battery profile, rc=%d\n",
				rc);
		...
		if (rc < 0)
			pr_err("Read jeita-scaling-min-fcc-ua from battery profile, rc=%d\n",
				rc);
		...
		pr_err("jeita-fcc-scaling: enabled = %d, jeita-fcc-scaling-temp-threshold = [%d, %d], jeita-scaling-min-fcc-ua = %ld, jeita-scaling-max_fcc_ua = %ld,jeita-fcc-step-size = %ld\n",
	...
	if (rc < 0) {
		if (rc == -ENODEV || rc == -EBUSY) {
			if (chip->get_config_retry_count++
				...
				pr_err("bms is not ready, retry: %d\n",
...
static unsigned int bChgCapacityflag = 1;
#ifdef SMT_VERSION
static unsigned int bChgCapacityTest = 70;
#else
static unsigned int bChgCapacityTest = 100;
#endif
...
static int handle_step_chg_config(struct step_chg_info *chip)
{
	...
	/* slave charger ic control start */
	int usb_charge_type, batt_status, batt_health, batt_soc, usb_voltage_max;
	struct smb_charger *chg = power_supply_get_drvdata(chip->batt_psy);

	if(chg == NULL){
		pr_err("ontim: chg is NULL\n");
		goto early_check;
	}
	if (!is_sgm_available(chip)){
		pr_info("ontim: handle sgm4151x-charger error!\n");
		goto early_check;
	}
	power_supply_get_property(chg->batt_psy, POWER_SUPPLY_PROP_STATUS, &pval);
	batt_status = pval.intval;
	//pr_info("ontim: batt_status = %d\n", batt_status);

	power_supply_get_property(chg->batt_psy, POWER_SUPPLY_PROP_HEALTH, &pval);
	batt_health = pval.intval; // GOOD == 1

	power_supply_get_property(chg->batt_psy, POWER_SUPPLY_PROP_CAPACITY, &pval);
	batt_soc = pval.intval;

	power_supply_get_property(chg->usb_psy, POWER_SUPPLY_PROP_POWER_NOW, &pval);
	usb_charge_type = pval.intval; // HVDCP3 == 21; PD == 9;
	//pr_info("ontim: charge_type = %d\n", usb_charge_type);

	power_supply_get_property(chg->usb_psy, POWER_SUPPLY_PROP_VOLTAGE_MAX, &pval);
	usb_voltage_max = pval.intval;
	//pr_info("ontim: usb_voltage_max = %d\n", usb_voltage_max);

	power_supply_get_property(chg->usb_psy, POWER_SUPPLY_PROP_CURRENT_NOW, &pval);
	//pr_info("ontim: usb_current_now = %d\n", pval.intval);

	if(bChgCapacityflag == 1){
		if((batt_status == POWER_SUPPLY_STATUS_CHARGING)&&(batt_health == POWER_SUPPLY_HEALTH_GOOD)){
			if(usb_charge_type == 9){
				if(chg->pd_active == QTI_POWER_SUPPLY_PD_ACTIVE){
					if(usb_voltage_max == 9000000){
						pval.intval = 1000000;
						power_supply_set_property(chip->sgm_psy, POWER_SUPPLY_PROP_INPUT_CURRENT_LIMIT, &pval);

						if(chg->thermal_mitigation[chg->system_temp_level] > 2500000){
							pval.intval = chg->thermal_mitigation[chg->system_temp_level] - 2500000;
							power_supply_set_property(chip->sgm_psy, POWER_SUPPLY_PROP_CONSTANT_CHARGE_CURRENT, &pval);
							pval.intval = 1;
							power_supply_set_property(chip->sgm_psy, POWER_SUPPLY_PROP_STATUS, &pval);
						}
						else{
							pval.intval = 0;
							power_supply_set_property(chip->sgm_psy, POWER_SUPPLY_PROP_STATUS, &pval);
						...
						pr_info("pd1: usb_voltage_max 5v slave charger keep disable\n");
				}
			}
			else if(usb_charge_type == 21){
				if(usb_voltage_max == 9000000){
					pval.intval = 1000000;
					power_supply_set_property(chip->sgm_psy, POWER_SUPPLY_PROP_INPUT_CURRENT_LIMIT, &pval);
					...
					pr_info("hvdcp3: usb_voltage_max 5v slave charger keep disable\n");
			}
			else{
				pval.intval = 0;
				power_supply_set_property(chip->sgm_psy, POWER_SUPPLY_PROP_STATUS, &pval);
				pr_info("charge type not fast slave charger disable\n");
			}
		}
		else{
			pval.intval = 0;
			power_supply_set_property(chip->sgm_psy, POWER_SUPPLY_PROP_STATUS, &pval);
			pr_info("ontim: batt_status not charging slave charger keep disable\n");
		}
	}
	else
		pr_info("ontim: bChgCapacityflag = 0 so do nothing\n");

	if((bChgCapacityTest <= batt_soc) && (bChgCapacityTest < 100)) {
		if(bChgCapacityflag == 1) {
			bChgCapacityflag = 0;
			pval.intval = 1;
			rc = power_supply_set_property(chip->batt_psy, POWER_SUPPLY_PROP_INPUT_CURRENT_LIMIT, &pval);
			if (rc)
				pr_err("ontim: pm7250b discharger enable error\n");

			pval.intval = 0;
			pr_info("ontim: sgm41513 charger discharger\n");
			power_supply_set_property(chip->sgm_psy, POWER_SUPPLY_PROP_STATUS, &pval);
		}
	} else {
		if(bChgCapacityflag == 0) {
			bChgCapacityflag = 1;
			pval.intval = 0;
			rc = power_supply_set_property(chip->batt_psy, POWER_SUPPLY_PROP_INPUT_CURRENT_LIMIT, &pval);
			if (rc)
				pr_err("ontim: pm7250b charger enable error\n");

			pval.intval = 1;
			pr_info("ontim: sgm41513 charger en\n");
			power_supply_set_property(chip->sgm_psy, POWER_SUPPLY_PROP_STATUS, &pval);
		}
	}
	/* slave charger ic control end */
early_check:
...
static void handle_jeita_fcc_scaling(struct step_chg_info *chip)
{
	...
	if (!is_between(chip->jeita_fcc_scaling_temp_threshold[0],
		...
		pr_err("jeita-fcc-scaling : Skip jeita scaling, temp out of range temp = %d\n",
			pval.intval);
	...
	if (first_time_entry ||
		...
		pr_err("jeita-fcc-scaling: first_time_entry = %d, max_fcc_ua = %ld, voted_fcc_ua = %d, temp_diff = %d, prev_temp = %d, current_temp = %d\n",
			first_time_entry, chip->jeita_max_fcc_ua, fcc_ua,
			temp_diff, chip->jeita_last_update_temp, pval.intval);

		chip->jeita_last_update_temp = pval.intval;
	} else {
		pr_err("jeita-fcc-scaling: Skip jeita mitigation temp within first_time_entry = %d, hys temp = %d, last_updated_temp = %d\n",
...
set_jeita_fv:
	...
	//pr_info("jeita: tbat = %d; fcc = %duA; fv = %duV;\n", pval.intval, fcc_ua, fv_uv);
...
static int hmd_charge_percentage_show(struct seq_file *m, void *v)
{
...
static ssize_t hmd_charge_percentage_write(struct file *file,
...
static int hmd_##name##_open(struct inode *node, struct file *file)	\
{									\
	return single_open(file, hmd_##name##_show, PDE_DATA(node));\
}									\
static const struct file_operations hmd_##name##_fops = {		\
	.owner = THIS_MODULE,						\
	.open = hmd_##name##_open,					\
	.read = seq_read,						\
	.llseek = seq_lseek,						\
	.release = single_release,					\
	.write = hmd_##name##_write,				\
}
PROC_FOPS_RW(charge_percentage);

static int step_chg_create_debugfs(void)
{
	struct proc_dir_entry *battery_dir = NULL;

	battery_dir = proc_mkdir("battery_cmd", NULL);
	if (!battery_dir) {
		pr_err("[%s]: mkdir /proc/battery_cmd failed\n", __func__);
		return -ENOMEM;
	}

	proc_create_data("hmd_battery_max_charge_percentage", 0640, battery_dir,
			&hmd_charge_percentage_fops, NULL);
	return 0;
}
