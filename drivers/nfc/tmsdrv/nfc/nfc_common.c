...
/* added by caiyongqing for Q5003-223 (20230101) begin */
static char nfc_version[20] = "unknown";
const struct capsule cmd_reset = {
    /* nci_cmd            len count */
    {"\x20\x00\x01\x0"},  4, 1,
};
/* added by caiyongqing for Q5003-223 (20230101) end */
...
static ssize_t nfcvendor_show(struct device *dev, struct device_attribute *attr,
                              char *buf)
{
    return (ssize_t)(snprintf(buf, MAX_CHIP_NAME_SIZE - 1, "%s\n", "tms-nfc-thn31f"));
}
...
/* added by caiyongqing for Q5003-223 (20230101) begin */
    __ATTR(vendor, 0444, vendor_show, NULL),
    __ATTR(nfcvendor, 0444, nfcvendor_show, NULL),
...
/* added by caiyongqing for Q5003-223 (20230101) begin */
bool core_reset(struct nfc_info *nfc, struct capsule *recv)
{
    ...
    nfc_hard_reset(nfc, GPIO_VEN_SET_WAIT_TIME_US);
    /*
    * there will be two responses, while fw version will be in the last response.
    */
    len = nfc_write(nfc, &cmd_reset);

    if (len != -ERROR) {
        len = nfc_read(nfc, recv, true);

        if (len == 4 && recv->nci_recv[0] == 0x40 && recv->nci_recv[1] == 0 && recv->nci_recv[2] == 0x01 ) {
            TMS_INFO("Core_reset success.\n");
        } else {
            TMS_ERR("Length of core Core_reset response_1 is wrong, expected 4 bytes, actually received %d bytes:", len);
            for (i = 0; i < len; i++) {
                TMS_INFO("Core_reset response_1[%d] = 0x%x", i, recv->nci_recv[i]);
            }
        }

        len = nfc_read(nfc, recv, true);        
        if (len != -ERROR) {
            if (recv->nci_recv[0] == 0x60 && ((recv->nci_recv[1] & 0x3F) == 0) && recv->nci_recv[2] != 0 
                && (recv->nci_recv[3] == 0x02 || recv->nci_recv[3] == 0x01)) {
                ret = true;
            }
        }
    }

    TMS_INFO("Core_reset end, ret = %d\n", ret);
    return ret;
}

void get_nfc_fw_version(struct nfc_info *nfc)
{
    bool ret = false;
    struct capsule recv;
    uint8_t recv_buf[MAX_NCI_BUFFER_SIZE];

    TMS_INFO("Start to get fw version:\n");
    recv.nci_recv = recv_buf;
    ret = core_reset(nfc, &recv);

    if (ret) {
        snprintf(nfc_version, sizeof(nfc_version), "fw:%x.%x.%x", recv.nci_recv[10], recv.nci_recv[11], recv.nci_recv[12]);
        TMS_INFO("Success to get fw version: %s\n", nfc_version);
    } else {
        snprintf(nfc_version, sizeof(nfc_version),"fw:D1.12.1B");
        TMS_ERR("Failed to get fw version, set to default value: fw:D1.12.1B.\n");
    }
}
/* added by caiyongqing for Q5003-223 (20230101) end */
