...
static ssize_t nfc_chip_name_show(struct device *dev, struct device_attribute *attr,
                              char *buf)
{
    return (ssize_t)(snprintf(buf, MAX_CHIP_NAME_SIZE - 1, "%s\n", "sec-nfc-s3nrn4v"));
}
static ssize_t nfc_bootloader_version_show(struct device *dev, struct device_attribute *attr,
                              char *buf)
{
    return (ssize_t)(snprintf(buf, MAX_CHIP_NAME_SIZE - 1, "%02x.%02x.%02x.%02x\n", rx_buff[4], rx_buff[5], rx_buff[6], rx_buff[7]));
}

static struct device_attribute nfc_attr[] = {
    __ATTR(nfc_chip_name, 0444, nfc_chip_name_show, NULL),
    __ATTR(nfc_bootloader_version_show, 0444, nfc_bootloader_version_show, NULL),
};
...
int nfc_create_sysfs_interfaces(struct device *dev)
{
    ...
    for (count = 0; count < ARRAY_SIZE(nfc_attr); count++) {
        if (device_create_file(dev, nfc_attr + count)) {
            pr_err("NFC create file failed\n");
            goto create_file_error;
        }
    }
    pr_info("Create sysfs interfaces success\n");
    return 0;
create_file_error:
    for (; count >= 0; count--) {
...
int sec_nfc_i2c_probe(struct i2c_client *client)
{
    ...
    /* Create system property node */
    ret = nfc_create_sysfs_interfaces(&client->dev);

    if (ret < 0) {
        pr_err("Create sysfs interface failed\n");
    }

    ret = sysfs_create_link(NULL, &client->dev.kobj, "sec_nfc");
