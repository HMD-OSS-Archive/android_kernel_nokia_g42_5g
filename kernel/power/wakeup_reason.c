...
// ning.wei++ begin for add more detail for wakeup reason
static char rtc_addition_info[MAX_SUSPEND_ABORT_LEN];
static char unknown_irq_wake_reason[MAX_SUSPEND_ABORT_LEN];
// ning.wei++ end for add more detail for wakeup reason
...
void clear_wakeup_reasons(void)
{
	...
	// ning.wei++ begin for add more detail for wakeup reason
	log_rtc_addition_info("%s", "None");
	log_unknown_irq_wake_reason("-1 %s", "unknown");
	// ning.wei++ end for add more detail for wakeup reason
...
void log_unknown_irq_wake_reason(const char *fmt, ...)
{
	va_list args;

	va_start(args, fmt);

	vsnprintf(unknown_irq_wake_reason, MAX_SUSPEND_ABORT_LEN, fmt, args);

	va_end(args);
}
EXPORT_SYMBOL_GPL(log_unknown_irq_wake_reason);
...
static void print_wakeup_sources(void)
{
	...
	if (wakeup_reason == RESUME_IRQ && !list_empty(&leaf_irqs))
		...
		pr_info("Resume cause unknown: %s\n", unknown_irq_wake_reason); // ning.wei++ modify
				       ...
				       struct kobj_attribute *attr, char *buf)
{
	...
	if (wakeup_reason == RESUME_IRQ && !list_empty(&leaf_irqs))
		...
		buf_offset = scnprintf(buf, PAGE_SIZE, "-1 %s",
				       non_irq_wake_reason);
	else { // ning.wei++ for add more detail for wakeup reason
		buf_offset += scnprintf(buf + buf_offset,
						PAGE_SIZE - buf_offset,
						"%s\n", unknown_irq_wake_reason);
	}
